      ...
      <table class="cart-list">
        ...
        <tbody>
          <tr v-for="(item, index) in cart" :key="index">
            <td>{{ item.name }}</td>
            <td>₱{{ item.cost }}</td>
            <td v-if="!item.editable">{{ item.quantity }}</td>
            <td v-else>
              <input class="edit-qty" type="number" v-model="item.newQuantity">
            </td>
            <td>
              <button v-if="!item.editable" class="remove-button" @click="removeItem(index)">Remove</button>
              | <button v-if="!item.editable" class="edit-button" @click="toggleEditing(index)">Edit Quantity</button>
